...
static int LogFileInit(LOG_FILE *pFile, int nType)
{
    ...
    /* 检查目录 */
    (void)strncpy(subDir + pathLen, g_strLogType[nType], strlen(g_strLogType[nType]));
...
static unsigned int FillTime(char *buf, unsigned int len)
{
    ...
    return (unsigned int)IVS_snprintf(buf, len, "%04d-%02d-%02d %02d:%02d:%02d.%03d%c%02d:%02d", 
